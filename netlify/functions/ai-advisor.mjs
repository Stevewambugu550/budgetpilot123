// BudgetPilot — AI Savings & Household Financial Advisor (Netlify Function)
// Mirrors the local Express endpoint from server.ts for serverless deployment.
// Uses Gemini when GEMINI_API_KEY is configured; otherwise serves instant
// rules-based financial insights computed from the submitted budget summary.
...
function getAi() {
  if (!aiClient && process.env.GEMINI_API_KEY) {
    aiClient = new GoogleGenAI({
      ...
      httpOptions: { headers: { 'User-Agent': 'budgetpilot-build' } },
    })
  }
  return aiClient
}
...
export const handler = async (event) => {
  ...
  const { budgetSummary, prompt, mode } = body
  ...
  try {
    ...
    if (!ai) {
      ...
      return { statusCode: 200, headers: CORS, body: JSON.stringify({ response: fallbackInsights, isFallback: true, mode }) }
    }
...
`
    ...
    if (isAiGenerated && responseText) {
      return { statusCode: 200, headers: CORS, body: JSON.stringify({ response: responseText, isFallback: false, mode }) }
    }
    ...
    return {
      statusCode: 200,
      headers: CORS,
      body: JSON.stringify({ response: fallbackInsights, isFallback: true, notice: 'Displaying live calculated financial diagnosis.', mode }),
    }
  }
}

function generateLocalFinancialInsights(budgetSummary, prompt, mode) {
  ...
  const overspentCats = categories.filter((c) => c.budgetCap > 0 && c.actualSpent > c.budgetCap)
  const highVarianceCats = categories.filter((c) => c.budgetCap > 0 && c.actualSpent >= c.budgetCap * 0.85 && c.actualSpent <= c.budgetCap)

  const bills = budgetSummary?.bills || []
  const sortedBills = [...bills].sort((a, b) => (b.amount || 0) - (a.amount || 0))
  ...
  if (mode === 'bill_negotiator') {
    const billItemsText = topBills.length > 0
      ? topBills.map((b) => `- **${b.name}** (${currency}${b.amount}/mo): Call provider, request loyalty discounts or bundle adjustments. Projected savings: **${currency}${Math.round(b.amount * 0.15)}–${currency}${Math.round(b.amount * 0.3)}/mo**`).join('\n')
...
*🏆 Projected 30-Day Household Gain: **~${currency}280–${currency}380***`
  ...
  const overspentWarning = overspentCats.length > 0
    ? `⚠️ **Over-Budget Envelopes Detected:** ${overspentCats.map((c) => `**${c.name}** (spent ${currency}${c.actualSpent} vs cap ${currency}${c.budgetCap})`).join(', ')}.`
  ...
  const highVarianceNotice = highVarianceCats.length > 0
    ? `\n- **Close to Limit (85%+ used):** ${highVarianceCats.map((c) => `${c.name} (${currency}${c.actualSpent}/${currency}${c.budgetCap})`).join(', ')}.`
    : ''
...
3. **Weekly Family Sync:** Spend 5 minutes every Sunday reviewing your **BudgetPilot Visuals** together to celebrate progress!`
}
